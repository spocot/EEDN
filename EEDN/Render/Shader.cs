...
namespace EEDN.Render
{
    public class Shader
    {
        ...
        public virtual void SetUniform(object value)
        {
            ...
            {
                object? val = i.GetValue(value);
                if (val is not null)
                    SetUniform(name + "_" + i.Name, val);
            }
        }
        ...
        public virtual void SetUniform(string name, object value)
        {
            ...
            {
                var val = (Color)value;
                GL.Uniform4(Uniforms[name], val.R / 255.0f, val.G / 255.0f, val.B / 255.0f, val.A / 255.0f);
            }
            else if (value.GetType() == typeof(Color4))
            {
                var val = (Color4)value;
                GL.Uniform4(Uniforms[name], val);
            }
            ...
            {
                throw new ArgumentException("Passed value type not supported");
            }
        }
